...
export class TablesService {
  processHumanMove(tableId: number, playerId: any, action: string, parsedAmount: number | undefined) {
  ...
  async actions(tableId: number, playerId: number, action: string, amount?: number) {
    ...
    switch (action) {
      case 'fold': return await this.fold(tableId, playerId);
      case 'call': return await this.call(tableId, playerId);
      case 'raise': return await this.raise(tableId, playerId, amount);
      case 'check': return await this.check(tableId, playerId);
      case 'leave': return this.leave(tableId, playerId);
      case 'startGame': return this.startGame(tableId, playerId);
      case 'small_blind': return this.blinds(tableId, playerId, 5);
      case 'big_blind': return this.blinds(tableId, playerId, 10);
      default: return { message: 'Action not found', possibleActions: [] };
    }
  }
  ...
  private formatResponse(tableId: number, playerId: number): { table: TableModel, possibleActions: string[] } {
    const table = this.tables[tableId];
    return {
      table,
  ...
  private getPossibleActions(table: TableModel, playerId: number): string[] {
    ...
    if (player.state !== 'fold') actions.push('check', 'call', 'raise', 'fold', 'leave');
    return actions;
  }
  

  async playRound(tableId: number): Promise<{ table: TableModel, possibleActions: string[] }> {
    const table = this.tables[tableId];
  ...
  checkGameEnd(tableId: number): string | undefined {
    ...
    if (active.length === 1) return `Everyone folded, player ${active[0].username} wins ${table.pot}`;
    return;
  }
